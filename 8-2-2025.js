...
document.addEventListener("DOMContentLoaded", () => {
    try {
        ...
        const logoUrlElement = document.getElementById('logo-url');
          if (logoUrlElement) {
             logoUrlElement.innerHTML = `<img src="${customization['logoUrl']}" alt="Logo" style="width: 50px; height: 50px; object-fit: contain; margin-right: 10px; display: block;" />`;
        }
        ...
        if(ctaBoxElement){
            ...
            const textColor = customization['textColor'] || '#333';
             const buttonBoxBgColor = customization['buttonBoxBgColor'] || '#f0f0f0';
                 ...
                 ctaBoxElement.style.left = customization['position'] === 'right' ? 'auto' : '20px';
                 ctaBoxElement.style.right = customization['position'] === 'right' ? '20px' : 'auto';
                       // Apply direction
           ctaBoxElement.classList.remove('ltr', 'rtl'); // Remove existing classes
         ctaBoxElement.classList.add( customization['direction'] || 'ltr');
              if (ctaElement)  ctaElement.style.backgroundColor = buttonBgColor
                   // Apply colors
