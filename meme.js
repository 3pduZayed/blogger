...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const API_ENDPOINT = 'https://3206b998e448.ngrok-free.app/search'; 
    ...
    async function fetchStudentData(query) {
        ...
        try {
            ...
            if (response.ok) {
                // If the response is a single student object (from ID search)
                if (data.seating_no && data.arabic_name && data.total_degree !== undefined) {
                    return { success: true, data: [data] }; // Wrap in array for consistent display
                } 
                // If the response is an object with a 'results' array (from name search or no results)
    ...
    // --- Determine Student Status (Success/Fail) based on total_degree ---
    // IMPORTANT: Adjust MAX_POSSIBLE_DEGREE and PASSING_DEGREE as per your system
    const MAX_POSSIBLE_DEGREE = 320; // Changed to 320 as per user request
    const PASSING_PERCENTAGE = 0.50; // 50% passing grade
    const PASSING_DEGREE = MAX_POSSIBLE_DEGREE * PASSING_PERCENTAGE; // Calculate passing degree

    const getStudentStatus = (totalDegree) => {
        if (totalDegree >= PASSING_DEGREE) {
            return 'ناجح';
        } else {
            return 'راسب';
        }
    ...
    const displayResults = (data) => {
        ...
        data.forEach(student => {
            ...
            studentNameCard.textContent = student.arabic_name; // Use new field name
    ...
    const showStudentDetailsModal = (student) => {
        ...
        // Determine student status using the new total_degree field
        const studentStatus = getStudentStatus(student.total_degree);
        const statusBadgeClass = studentStatus === 'ناجح' ? 'success' : 'failed';

        let detailsHtml = `
            <div class="sharable-image-content">
                <h3 class="text-3xl font-bold mb-3">${student.arabic_name}</h3> <!-- Use new field name -->
                <p><strong>رقم الجلوس:</strong> ${student.seating_no}</p> <!-- Use new field name -->
                <p><strong>إجمالي الدرجات:</strong> ${student.total_degree}</p> <!-- Use new field name -->
        ...
        if (studentStatus === 'ناجح') {
            detailsHtml += `
                <div class="success-message mt-4">
                    <p class="text-2xl mb-2">🎉 تهانينا، ${student.arabic_name}! أنت ناجح! 🎉</p>
