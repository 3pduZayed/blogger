...
function updateIframeSource(pageUrl) {
    ...
    if (isBlocked) {
        // الموقع محظور، اعرض صفحة المؤقت والإعلانات
        console.log('Site blocked (manual list):', pageUrl);
        showTimerPage(iframe, pageUrl);
    } else {
        // الموقع ليس محظورًا، حاول تحميله وأخفي كل شيء آخر
        console.log('Attempting to load site:', pageUrl);
        try {
            iframe.src = pageUrl;
            hideAllExceptIframe(); // استدعاء الدالة لإخفاء العناصر الأخرى
        } catch (e) {
...
// دالة لإخفاء كل شيء ما عدا الـ iframe
...
function showCtaBox() {
    ...
    if (ctaBox) {
        ...
        ctaBox.style.display = 'block'; // تأكد من أن العنصر ظاهر
    }
}
