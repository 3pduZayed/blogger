...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (adPopup) {
        ...
        if (openButton) {
            openButton.textContent = customization['popupCtaText'] || 'فتح'; // استخدام المتغير المخصص
            openButton.addEventListener('click', () => {
                window.open(customization['popupCtaUrl'], '_blank'); // استخدام الرابط المخصص
            });
        }
        ...
        if (popupContentElement) {
            popupContentElement.innerHTML = customization['popupContent'] || '<p>إعلان منبثق افتراضي.</p>'; // استخدام النص المخصص
        }
    }
});
